...
def train_spacy_model(
    ...
    if training_data_path is None:
        training_data_path = root_dir / configs["trainer"]["training_data_path"]
    ...
    with mlflow.start_run(
        run_name=mlflow_conf["run"]["name"], experiment_id=experiment.experiment_id
    ) as run:
        ...
        registered_model = client.get_registered_model(name=mlflow_conf["run"]["registered_model_name"])
        model_version = registered_model.latest_versions[0].version
        
        client.set_registered_model_alias(name=mlflow_conf["run"]["registered_model_name"], alias="Champion", version=model_version)
